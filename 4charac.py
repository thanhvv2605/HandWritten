...
torch.save(model.state_dict(), 'mymodel3charac.pth')
# Khởi tạo lại kiến trúc mô hình
model = ConvNet()

# Tải tham số vào mô hình
model.load_state_dict(torch.load('mymodel3charac.pth', weights_only=True))
...
print("End of training, model saved")
...
#     model.load_state_dict(torch.load('mymodel4charac.pth', weights_only=True))  # Tải trọng số đã huấn luyện

#     # Đọc và xử lý hình ảnh đầu vào
#     image_path = 'TestA.jpg'  # Thay bằng đường dẫn đến hình ảnh của bạn
